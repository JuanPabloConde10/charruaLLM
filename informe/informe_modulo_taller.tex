...
prensa uruguaya, y entrenó desde cero variantes cased y uncased de un modelo
...
magnitud de 0,1 mil millones de parámetros
...
\begin{enumerate}
    \item Recolección y preparación del corpus de noticias uruguayas.
    ...
    \item Preparación de un corpus general en español y entrenamiento de
          \spanishmodel.
    \item Creación de un dataset de evaluación a partir de afirmaciones de
          QuUANTO, derivadas originalmente de QuALES.
...
3.220.405.289 bytes. Debido a su tamaño, se preparó mediante streaming,
...
Se partió del archivo \code{QuUANTO\_unified\_final.json}. Según su metadato,
QuUANTO contiene afirmaciones derivadas de pares de pregunta-respuesta de
QuALES, validadas mediante un procedimiento multiagente y revisión humana.
QuALES fue una tarea de Question Answering extractivo en español organizada en
IberLEF 2022 y basada en noticias uruguayas sobre la pandemia de COVID-19 y
temas relacionados~\cite{rosa2022}.
...
\code{final\_dataset.csv}. El dataset contiene 834 ejemplos: 319 del split
train original de QuALES, 261 de \code{dev\_gold} y 254 de
\code{test\_gold}.

Los nombres de los splits originales se conservaron. La evaluación principal
se ejecutó sobre los 834 ejemplos y también se calcularon resultados separados
por split. Esta transformación evalúa conocimiento léxico y capacidad de
...
\begin{table}[H]
    \centering
    \caption{Exact matching por split original de QuALES.}
    \label{tab:exact-split}
    \begin{tabular}{llrr}
        \toprule
        Modelo & Split & Aciertos/total & Exact matching \\
        \midrule
        \charrua & \code{dev\_gold} & 71/261 & 27,20\,\% \\
        \charrua & \code{test\_gold} & 71/254 & 27,95\,\% \\
        \charrua & train & 80/319 & 25,08\,\% \\
        \midrule
        \spanishmodel & \code{dev\_gold} & 13/261 & 4,98\,\% \\
        \spanishmodel & \code{test\_gold} & 16/254 & 6,30\,\% \\
        \spanishmodel & train & 10/319 & 3,13\,\% \\
        \bottomrule
    \end{tabular}
\end{table}

En el conjunto completo, \charrua{} supera a \spanishmodel{} por 21,94 puntos
porcentuales y obtiene aproximadamente 5,69 veces su tasa de acierto. La
ventaja aparece en cada split y es especialmente relevante en
\code{test\_gold}, donde alcanza 27,95\,\% frente a 6,30\,\%.

Algunos errores no implican una continuación lingüísticamente absurda. Por
ejemplo, para ``El Pereira Rossell es un hospital'', \charrua{} predijo
``centro''; y para oraciones terminadas en ``desempleo'' predijo ``paro''.
Exact matching penaliza estas alternativas aunque estén relacionadas
semánticamente. Esta es una de las razones para complementar la evaluación con
NLL.
...
\begin{table}[H]
    \centering
    \small
    \caption{Resultados de NLL en el split \code{test\_gold}.}
    \label{tab:nll-test}
    \begin{tabular}{lrrrr}
        \toprule
        Modelo & NLL/palabra & NLL/token & Perplexity/token & Prob. palabra \\
        \midrule
        \charrua & 3,7852 & 2,0587 & 7,84 & 25,56\,\% \\
        \spanishmodel & 24,6254 & 7,7220 & 2.257,54 & 3,84\,\% \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    \item El dataset contiene 834 ejemplos, por lo que convendría ampliarlo.
    \item La evaluación completa incluye ejemplos con etiquetas train,
          \code{dev\_gold} y \code{test\_gold} de QuALES. Para una cifra
          principal estrictamente held-out debería priorizarse
          \code{test\_gold}.
    \item QuALES y el corpus de \charrua{} proceden de noticias uruguayas. No
          se realizó una auditoría de solapamiento textual entre los documentos
          de preentrenamiento y las afirmaciones de evaluación, por lo que no
          puede descartarse leakage.
    \item Exact matching no reconoce sinónimos ni equivalencias morfológicas.
    \item Los resultados de masked-word prediction de ROUberta no son
          directamente comparables con los nuestros: su encoder observa ambos
          lados de la máscara, mientras nuestro decoder solo observa el prefijo.
    \item Los tokenizers distintos dificultan la comparación directa de
          perplexity.
    \item La conversión a minúsculas de \spanishmodel{} introduce una
          diferencia adicional además del dominio de los datos.
    \item Sería útil entrenar un control con el mismo tokenizer para ambos
          corpus y otro \spanishmodel{} sin conversión a minúsculas.
    \item También sería útil reportar top-$k$ accuracy, mean reciprocal rank,
          bits por carácter y calibración de probabilidades.
    \item Las reglas de escalamiento de Chinchilla motivan probar modelos de
          mayor tamaño con menos pasos para comparar eficiencia de cómputo.
\end{itemize}
...
\spanishmodel. En \code{test\_gold} obtuvo 27,95\,\% frente a 6,30\,\%.
También alcanzó una NLL media por palabra considerablemente menor: 4,1396
