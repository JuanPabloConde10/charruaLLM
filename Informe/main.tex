...
continuaciones. Algunos errores de \charrua{} son palabras relacionadas con el
objetivo y resultan razonables en el contexto. Por ejemplo, ante la afirmación
``El Pereira Rossell es un hospital'', el modelo predijo ``centro''; y ante
oraciones cuyo objetivo era ``desempleo'', predijo ``paro''. Estas respuestas no
coinciden con la cadena esperada y por eso cuentan como errores de exact matching,
pero muestran que el modelo puede recuperar relaciones léxicas y semánticas del
dominio periodístico.

Este tipo de casos justifica complementar la métrica exacta con NLL. La primera
responde si el modelo generó exactamente la palabra de referencia, mientras que
la segunda permite medir cuánta probabilidad asignó a esa palabra aunque otra
continuación haya quedado en primer lugar. El análisis también debe considerar
que \spanishmodel{} fue entrenado con texto en minúsculas, mientras que las
afirmaciones conservan mayúsculas y nombres propios. Por ello, algunos errores
pueden reflejar una diferencia de representación además de una falta de
